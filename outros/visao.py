    ...
    def match_template(self):
        ...
        for idx, roi in enumerate(self.roi_list):
            ...
            try:
                ...
                #teste = self.is_image_equal(roi_image, template_image, min_matches=10)
                polaridade = self.detect_polarity(roi_image,show_pol=True)
                print("Polaridade dessa bagaça",polaridade)
                print("Valor",max_val)
                if max_val >= 0.80:  # Limite de confiança
                    color = (0, 255, 0)  # Verde
                elif max_val < 0.80 and max_val >=0.30:
                    color = (0, 254, 255)
                else:
                    color = (0, 0, 255)
