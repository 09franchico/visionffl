    ...
    def match_template(self):
        ...
        for idx, roi in enumerate(self.roi_list):
            ...
            try:
                ...
                # imagem  = self.find_gray_and_white_components(roi_image)
                # cv2.imshow("Detected Circle", imagem)
                # cv2.waitKey(0)
                #EMMM TESTE
                
                # polaridae = self.detect_polarity(roi_image,show_pol=True)
        ...
        for i in range(1, num_labels):  # começa do 1 para ignorar o fundo (label 0)
            ...
            if min_area <= area <= max_area:
                ...
                if contains_gray and contains_white:
                    cv2.rectangle(image_data, (x, y), (x + w, y + h), (255, 0, 0), 1)
